//! Probe PSM type 0x0061 (igArc2d candidate).
//! If this surfaces real records with clean geometry, it would
//! resolve the 0x0030 GArc2d mystery from Slice F-I.
...
fn probe_fixture(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Check ALL plausible candidates for "arc" / "circle" / "rectangle":
    // 0x0020 = igRectangle2d
    // 0x0059 = igCircle2d
    // 0x0061 = igArc2d
    // 0x007E = igEllipticalArc2d
    let candidates = [0x0020u16, 0x0059, 0x0061, 0x007E];
    let header_len = 6;
    ...
    for target in candidates {
        ...
        for off in 0..=max_offset {
            ...
            *size_dist.entry(bytes_to_follow).or_insert(0) += 1;
            ...
            if !first_dump_done && hits == 1 {
                ...
                println!(
                    "  HIT[0x{:04X}] @ 0x{:06x} bytes_to_follow={} payload_len={}",
                    target, off, bytes_to_follow, payload_len
                );
                let dump_len = payload_len.min(96);
        ...
        if hits > 0 {
            println!(
                "  type 0x{:04X}: {} hits, sizes: {:?}",
                target, hits, size_dist
            );
        }
    }
    Ok(())
