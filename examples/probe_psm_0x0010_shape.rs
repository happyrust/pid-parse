//! Probe PSM type 0x0010 — 638 cross-fixture hits, the most
//! prevalent yet-unexplained record type after Slice J-N.
...
fn probe_fixture(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  type 0x0010: {} hits", hits);
    let mut sorted: Vec<_> = size_dist.iter().collect();
    sorted.sort_by_key(|(_, c)| std::cmp::Reverse(**c));
    for (size, count) in sorted.iter().take(8) {
