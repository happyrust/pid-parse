...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let fixture = std::env::args().nth(1).unwrap_or_else(|| "test-file/DWG-0201GP06-01.pid".to_string());
    ...
    for &off in &[0x001195usize, 0x001439, 0x0015f6, 0x003676, 0x00379c] {
        let inner = off + 18;
        if inner + 64 > bytes.len() { continue; }
        ...
        for (label, field_off) in [
            ...
            if field_off == 40 {
                ...
                let raw = &bytes[p..p+8];
                let f = f64::from_le_bytes([raw[0],raw[1],raw[2],raw[3],raw[4],raw[5],raw[6],raw[7]]);
                println!("  {label} : {:?} bits=0x{:016X} f64={:+.10e}",
                    raw, u64::from_le_bytes([raw[0],raw[1],raw[2],raw[3],raw[4],raw[5],raw[6],raw[7]]), f);
            }
        }
    }
    Ok(())
