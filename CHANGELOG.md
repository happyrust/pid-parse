...
- **解出来的值本身给了一条线索，当天就验了。** 12 个圆里 10 个圆心在 (0, 0)，半径 1.27 /
  1.59 / 6.35 / 7.57 mm——6.35 mm 正是 1/2" 仪表圈的半径；弧成对出现。这是围着原点画的
  **符号字形**，不是页面内容。拿同一张图放置的符号的 `.sym` 本体逐值比对
  （`examples/probe_nested_site_curves_are_symbol_bodies.rs`）：**24 条里 20 条在 1 nm 内
  与某个本体的圆 / 弧完全相同**，对上的每个符号其本体全部圆 / 弧都在存储里（D06 3 个符号、
  0201 6 个、0202 4 个）；剩 4 条是疑似参数化重算的 Manifold（`/JSite396`）和库里没有的
  `Xa*` 符号（工艺图），没有一条矛盾。结论：嵌套 `LdcSite` 是图纸**内嵌的符号定义缓存**，
  坐标是符号本地坐标，08-31 要找的"到页面的变换"在这一层本来就不存在——变换是引用它的
  `igSymbol2d` 放置自己带的；剩下要解的是**定义 ↔ 实例的链接边**。屏幕上这 20 条其实早就由
  外部 `.sym` 库画出来了；缓存的价值在**没有库、参数化符号、站点自定义符号**三种情形。
  见 `docs/analysis/2026-09-07-nested-site-curves-are-embedded-symbol-bodies.md`。
...
- **长形的字段解开了，二分是完美的。** `+14 == 0x0010` 决定 44 字节头后面有没有连接
  点表：`u32 点数` + 每点 `{u8 1; u32 1; f64; u16 字符数; UTF-16 名字; u32 被指的
  0x00C7 oid}`。8 条长形按这个读法**吃到最后一个字节、零剩余**，24 个点只有
  `Left`/`Right`/`Bottom`/`Top` 四个名字，点上的 `f64` 与所指 `0x00C7` 自己 `+12` 的
  `f64` **12/12** 相等（另 12 个点所在存储没有 `0x00C7`）。于是 203 条 `0x00C7`
  **恰好二分**：12 条被活着的长形列着、191 条带无记录引用者，**重叠 0、遗漏 0**
...
- **`0x006F` 也解开了，这一族齐了。** 13 条全部字节精确：常量 GUID + `JBExpression
  object` CLSID（`+38`）+ `Double Value Object` CLSID（`+58`，值类型）+ `u32` 长度的
  ASCII 签名 `%>i%<i`（`%>` 出参 / `%<` 入参）+ 每个 `%` 一个 `{u32 oid, 接口 GUID}`
  槽 + 收尾的 UTF-16 公式。**出参 13/13 是 `0x0115` JDim**，入参是 **12 条 `0x00C7`
  Double Value**——正是 §4.1 二分里「被活着的长形列着」的那 12 条。公式四种：
  `0E$1`×8、`0E$1+0.01`×2、`0E$1+0.1`×2、`0E($1+$2)/10`×1。于是每条 `0x00C7` 的两个
  引用者是两件事：**符号信息给它起名，关系把它喂进尺寸**。`JSite793` 连一条
...
- **挂上了 `JSite::symbol_information`**（不是 `ClusterInfo`：`parse_clusters` 只开
  ...
  全都带这一族。
- probe `examples/probe_symbol_information_family_shape.rs`：按**声明长度走链**再与
  解码器对帐，多验出一条——**每条被接受的记录都正好落在一个链记录的起止上，零例外**
  （逐字节滑窗没蒙出假记录，变长尾也没差一个字节）。顺带修掉一处 rustdoc 链到私有
  常量导致 `cargo rustdoc` 直接失败的问题（失败时它不输出 missing-docs 警告，所以
  按「数警告条数」写的检查会误读成 0；现在 rustdoc 退出码本身也在核）。
...
### 行距 `+66` 接线：四条多行标签不再首尾相接铺成一行（2026-08-22）

- **先量后接**。`+66` 的证据早已是 native-reader，但行距只对多行标签有意义，而「语料里到底有没有
  多行标签」没人量过。新 probe `examples/probe_text_multiline_census`：链上 235 条 igTextBox
  **全部被解码器接收、denominator 零缺口**（先立住这条，否则「没多行」可能只是拒收造成的假象），
  其中 **4 条带换行**，断行符是单独的 `U+000D`，无 LF。
- **决定接线的不是那个 4，是交叉表**：多行 4 条**全部**声明 `1.5`，声明 `1.0` 的 228 条里
  **一条多行都没有**。偏移读错也能产出某个分布，但产不出和「哪些标签真有第二行」对得上的分布。
  ratchet 因此钉的是这个**对应关系**（`{(单行,1.0):151, (单行,1.5):3, (多行,1.5):2}`）而不是计数。
- 合理区间取 `0.25..=4.0`——**不是我们定的**，是 DXF 给 MTEXT 行距因子（group 44）的范围，
  也正是消费侧要填的字段。语料第三个值 `0.0` 按构造落在区间外（零倍行距会把每行摞在第一行上），
  读作「未声明」。
- **顺带查出这 4 条今天就是画错的，且与行距无关**：`OpenCADStudio` 把每条标签导入成一个 DXF
  TEXT，而 TEXT 按定义单行；`text_support::text_local_bounds` 用一个水平游标遍历 chars，
  无任何换行处理，`U+000D` 无字形只把游标右推。四行图签是首尾相接横铺出去的。所以 OCS 侧
  新增 `stack_text_lines`，在样式应用**之前**把带断行的标签拆成每行一个 TEXT，沿**基线法向**
  （不是垂直向下，语料有 0°/90°/180° 标签）按 `字高 × 行距` 逐行下移。
- **顺带查实一处 join 碰撞**：`工艺管道及仪表流程-1.pid` 的 `/Sheet6` 上**两条 igTextBox 共用
  oid 6345**。`TextHeightIndex` 以 `(stream, oid)` 为键，两条塌成一条、第二条静默继承第一条的
  样式。本轮零影响（两条都声明 1.0、都非多行），但**这把钥匙线宽 / 填充 / 文字三张索引都在用**，
  下次谁拿它做 join 都会踩。已在 ratchet 注释里钉住。
- 口径声明：这些数都是**两跳口径**（`igTextBox +14` → `JStyleTextPara +38` → `JStyleTextChar`）
  下算出来的。行距本身不受影响（行距是段落属性），但 `字高 × 行距` 的行间距受影响，因为那个
  字高来自两跳。见下一条。
  证据：`docs/analysis/2026-08-22-four-labels-have-a-second-line.md`。

### 原生读序坐实 igTextBox 尾块与富文本 run：字节账目全语料闭合（2026-08-22）

- **`bytes_to_follow = 22 公共头 + body_len + tail_len`，全语料 260/260 精确闭合、零余量。**
  `body_len` 形状 1 为 `2+2*count`、形状 2 为 `10+2*count`、形状 3 为 `6+2*count+8*(A+B)`；
  `tail_len` kind 1 为 36，kind 2 为 `68 + 8*popcount(flags & 0x3F800000)`。
- **三处互证，不是单看 Load 反推**：`IGDSFactoryText::GetSize`（`sub_56498AB0`）把头+body 与
  尾块自报尺寸相加；`Save`（`sub_56498940`）把「总长 − 6」写进记录字节 `2..5`，那正是 btf 本身；
  `Save` 结尾还把尾块自报的 kind 写回 payload `+20`。
- **`+20` 不是独立字段**，是放置对象自报的尾块格式。**形状判别位也不是独立字段**，是从 run 数量
  推出来的（无 run = 1，恰好一条选择子 1 = 2，其余 = 3），全语料 260/260 复现。**因此
  「形状 2 的 `count|0x10000` 是标记」这个旧说法从来不成立**——那 4 字节就是一条 run 条目，
  `0x10000` 是它高半字里的选择子 1。
- **kind 2 多挂的 32／40 字节解开**：尾块由 `IGDSFactoryTextPointRectShape` 读，kind 1 只读 36
  字节，kind 2 读 68 字节固定部分再按 `tail+36` 的标志位每置一位多读 8 字节。`68−36=32`、
  `76−36=40` 正是此前记的那两个数；七个可选位合起来正好 `0x3F800000`，而「是不是 kind 2」的
  判据（`sub_56497A10`）用的是同一个掩码，自洽。
- **形状 3 的 A／B 是格式化 run**：每条 8 字节 `(u16 长度, u16 选择子, u32 样式id)`，选择子 1 →
  `0x002C`（320 条里 313 条命中），选择子 2 → `0x002D`（13/13，且全部等于记录自己的 `+14`）。
  `GetSize` 有硬不变式：选择子 1 的长度之和必须等于字符数，实测 228/228。实例：管道号
  `250-LNG-57602- - ` 的号段与分隔符交替用两条不同字符样式。
- **一条待查实的口径问题**（优先级高于接线 run）：形状 2 那条 run **直接指一条 `JStyleTextChar`**，
  而本仓解码器走两跳。215 条里 **153 条两者不是同一条**；再比这 153 条实际产出的字形，
  **147 条字高或字体真的不同、0 条相同、6 条解析不出**——「id 不同」几乎必然带来可见差异，
  不是同一套值换了个编号。**字体名都能换**（run `Arial Narrow` 对 para `Arial`），不只是字号。
  run 侧是非整数（实际值），para 侧是 2.5 / 3.175 这类标称值，而原生把 run 交给
  `AddCharStyleRun`——那才是真正给字上格式的调用。**本轮不下结论**：读序只证明 run 被逐区间
  应用，没读到渲染端的取舍规则。
- 仍未解（均已计入长度、不构成账目缺口）：`tail+32` dword、kind 2 标志位的高两位与 `0x4`／`0x8`、
  `tail+40`／`+48` 两个形似宽高的 double、`tail+56/60/64` 三个 dword。
  证据：`docs/analysis/2026-08-22-igtextbox-tail-kind-2-and-formatting-runs.md`，
  复现见 `examples/probe_igtextbox_tail_and_runs` 与 `tools/idalib_igtextbox_style_apply.py`。
...
- **序列化器是 `style.dll!sub_10030A20`**。第一轮从 DoIO 调用点反推失败；改从
  `IJStyleTextCharImp` 的 get/put 访问器（那批 21–24 字节的小函数）反查对象槽位
  才找到——与当初定位填充色同一手法。
- **`+34` 文字颜色升到 native-reader**：读序把它夹在两个**此前已是 native-reader**
  的锚点之间（`+30` 语言字段有 `GetKeyboardLayout(0)` 兜底、`+42` 字高），而
  `30 + 2 + 2 + 4 + 4 = 42` **精确闭合、零余量**，颜色只能在 `+34`。
- **「`-1` 哨兵」之谜解开**：它确实存在，但只在内存里——序列化器在 IO 前后各有一次
  `-1 → 0` 归一化，**所以盘上永不出现 `-1`**。这正是实测 381 条零个 `-1`、367 条为
  `0` 的原因。把 `0` 读作黑色与原生行为完全一致。
- **字体名定位**：`+68` u16 长度 + `+70` UTF-16 正文，`payload == 70 + 2*len`
  **全语料 381/381 成立**，`+70` 读出 `Arial` 111、`Arial Narrow` 108、`宋体` 79、
  `仿宋` 26 等真字体名，零条不可读。此前的「最长 UTF-16 串」启发式扫的是整个
  payload，产出乱码。指南两条缺口就此关闭。
...
- **`JStyleTextChar +34` 是文字颜色**，Win32 `COLORREF` `0x00BBGGRR`，与
  `JStyleSimpleLine +50`、`JStyleSimpleFill +30` 同编码。此前无人读它，所以每一条
  标签都渲染成 `PID-TEXT` 图层的绿色——那是导入器自己发明的颜色。
- **指南的「带 `-1` 哨兵」经实测不成立**：全语料 381 条 `0x002C` 记录里**零个** `-1`，
  而 **381/381 高字节为零**。不受约束的 `u32` 每条约 1/256 的机会高字节为零，381 条
  全中意味着该字段构造上就是 24 位——正是 `COLORREF`。佐证：`#FE0060` 是**同一张图
  的线条也在用**的非常规色值（OCS 的 import 测试钉着 DWG-0201 线条调色板含它）。
- **黑是被声明的颜色，不是"没声明"**：367 条明写 `#000000`。渲染器的 `adapt_to_bg`
  在深色背景上把它翻成白，与图纸自己的黑色线条待遇一致。
- **证据等级 corpus + 结构不变量，不是 native-reader**：`style.dll` 里
  `JStyleTextChar` 的序列化器在时间盒内没定位到（vtable 虚函数无一直接调 DoIO），
  如实标注。同一条记录上 `+42` 字高是 native-reader，`+34` 不是。
- 实现：`ResolvedTextHeight` 多带 `colour` 与 `rgb()`，**不新建第二张索引表**——颜色
  与字高共用同一条两跳链，因此**同生共死**（字高被拒的记录也拿不到颜色）。
  `OpenCADStudio` 新增 `apply_text_colour`，作用域限于 `PID-TEXT`，**未放宽**
  `apply_symbology` 的图层门禁。
- 证据：`docs/analysis/2026-08-13-text-colour-is-002c-plus-34.md`；指南 §8.2 已更新。
