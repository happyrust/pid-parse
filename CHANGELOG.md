...
- **一条待查实的口径问题**（优先级高于接线 run）：形状 2 那条 run **直接指一条 `JStyleTextChar`**，
  而本仓解码器走两跳。215 条里 **153 条两者不是同一条**；再比实际字形（该项测到 201 条语料）
  122 条不一致里 **116 条字高／字体真的不同**（如 run `Arial` 2.2931mm vs para `Arial`
  3.1750mm）。run 侧是非整数（实际值），para 侧是标称值，而原生把 run 交给 `AddCharStyleRun`
  ——那才是真正给字上格式的调用。**本轮不下结论**：读序只证明 run 被逐区间应用，没读到渲染端的
  取舍规则。
