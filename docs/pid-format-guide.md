...
| 188 | `0x0115` | 多处 | 42/46 |
| 184 | `0x0057` / `0x0060`（ViewFilterSet）| **多数无 payload 对应** | 60/366 |
...
`JSite` 条目形状一致——`[(2, tag 182), (X, tag 225|261)]`——含义是**两条入边**：
对象 2（`/JSitesList` 列表对象，见下）引用它，一个 `0x004F` 记录引用它。`JSite` 的
id 从不出现在任何 value 里 = `JSite` 自己不发出这张表索引的引用。

**顺带破掉的 `/JSitesList` 流**：`'OLEM'` magic + `u32 数量` + `u32 id 表`。D06 那份
是 `[23, 204, 300, 336, 1835, 2605, 3644, 145, 151]`——恰好是全部持有 tag 181 成员的
条目加两个 Document site。**四图恒定的引用者 id 2 就是这个列表对象**：它没有 PSM
记录，它的引用直接存在这条流里，`JSite` 条目上的 `(2, 182)` 就是它的入边。所以
`JSite` 不止 Server/Imagineer 文档——JSitesList 里每个 id 都是一个 OLE 站点对象
（符号/框架通过 `+29`/`+156` 挂在上面）。
...
`_SupportOnlyList` 的 id 不是四图都一样，所以对上的是**名字**不是号。182 还盖着
`/JSitesList`（id 2）等别的列表/容器对象，是 13 个 tag 里唯一明显跨家族的。
