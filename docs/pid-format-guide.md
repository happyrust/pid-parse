...
### 3.2 `PSMspacemap` 存的是对象引用图（2026-08-27）

**等级：corpus（全语料实测，无例外）。** 上一节的帧来自反汇编；这一节的三条字段
语义来自四张图 38 个 member 流的穷举，probe 是
`probe_psmspacemap_tag_and_span`，分析见
...
**2. `value` 是同一 id 空间里的一个活对象的持久 id。** 3989 个非空成员里 3988 个落在
「小于该段 `m_iNext` 且不在自由表里」的活索引上，0 个越界；而这些段发出过的 40893 个
索引里有 11998 个（29%）在自由表上——蒙不出这个数。唯一的例外是
`DWG-0201GP06-01.pid` 顶层 `0x00006000` 里 entry 2 指向 369（tag 249），那个索引已被
回收，是一条悬空边。
...
**3. `tag` 是被指对象的类，不是这条引用的角色。** 判据是纯表内的：语料里被指过的
2415 个对象，**每一个身上的 tag 都只有一种**，包括被指了不止一次的 675 个，零分歧。
如果 tag 说的是「这条引用干什么用」，同一个对象被不同角色指到就该出现分歧。

另外三条旁证同向：

- 同一个低号 id 在四张互不相干的图里拿同一个 tag——`2 → 182`、`7 → 183`、
  `19 → 184`，四图全中。
- 在有自己条目的目标里，tag 几乎钉死目标条目的 head 高半：`183`/`184`/`190` 只落
  在 2，`181`/`188`/`249` 只落在 3；只有 `182` 两边都有（128 比 12）。
