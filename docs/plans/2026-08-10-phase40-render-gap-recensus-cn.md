...
S1  [pid-parse] 四分账重普查 + 证伪 + 规则归因            ✅ 2026-08-10 完成
 ...
 -> S4  [pid-parse] 先证「JSite 内的线该不该画在图纸上」   ⬜ 未开始（S5 的前置）
...
45 条 text 占静默桶近三分之一，病因未知就无法与线排序。手法同 S1 的规则复算：逐条
复算 `igTextBox` / `igLineString2d` 的校验链，看是哪一道在拒、分几个总体。
**先归因，再决定做不做。**

### S4：先证「JSite 内的线该不该画」 ⬜

A01 的 80 条在 `/JSite204/Sheet6`——一个 JSite（符号站点）内的 sheet。它们可能是
符号局部几何，本就不该出现在图纸坐标系里。**这是 S5 的前置**：同 Phase 39 S3 的
规矩，先证消费者存在，再谈解码。若证不成立，S5 的收益就只剩 DWG-0202 的 8 条。

### S5：解 `igLine2d` 的第二种帧装 ⬜

手法同 `0x002A`/`0x002E`/`0x002F` 已验证四次的路子：`radsrvitem` 的 `igLine2d`
序列化器定字段与语义（`remaining_header` 到底是什么、两个总体是不是两个版本），
语料定字节偏移。**不得在证明坐标偏移之前先按标准偏移读数上屏**——那正是
「看着像真的」。
