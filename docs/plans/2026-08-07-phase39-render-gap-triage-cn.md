...
> **2026-08-10 更正**：这句错了。`0x0013 igBoundary2d` 就是那个面族，它被记在
> 「已解码族占满」那一栏里、又被标注「故意不发」，于是在找「填充的消费者」时被跳过。

**StyleCluster 侧填充样式**（格式指南 §5 记录表）：`0x002A JStyleSimpleFill` 25 条、
`0x002B JStyleHatchFill` 11 条——**样式定义在，但没有几何去引用它们**（Sheet 流里没有
被填充的面）。
> **2026-08-10 更正**：也错了。全语料 20 条 `igBoundary2d` 全部经 `JStyleOverride`
> 引用一条 `0x002A`。链式精确计数下填充样式是 22 条而非 36。见 §1 E。
...
### E — 填充 `0x002A` / `0x002B`（**已推翻：消费者一直在**）

**原判**：可见占比 ≈ 0，六图里没有一条被填充的几何，36 条填充样式无消费者；
本期不做，写 negative note 封存。

**2026-08-10 实测后推翻**（`probe_fill_style_consumers`）：

- **`0x0013 igBoundary2d` 就是那个「面」族，全语料 20 条边界，20 条**都经
  `JStyleOverride` 指到一条 `0x002A JStyleSimpleFill`。同一轮里 326 条 `igLine2d`、
  153 条 `igLineString2d`、186 条 `igPoint2d`、184 条 `igTextBox` **一条都不沾**
  ——按族干净分层，不是巧合。
- 链路：DWG-0202 override 21 → `0x002A` id 20（5 条边界）；工艺管道-1 override 24
  → `0x002A` id 23（10 条边界）。被引用的全是实心 `0x002A`，`0x002B` 图案填充
  一条未被引用。
- 「无消费者」是 DWG-0201 / D06 / A01 三张图的**局部**事实，被当成了全语料结论。
- **可见占比不是 0**：两张图上 15 个被填充的封闭区域，现在在 OCS 里一个像素都不画
  ——`igBoundary2d` 的 emitter 是注册过的 no-op（理由：与成员线重复）。
- 顺带：填充样式的链式精确计数是 **22** 条，不是指南 §5 的 36。差值未解释，
  假设是扫描式计数与链式计数的差异（同 `0x3FE6` 那一课），**未验证前不要改指南**。
- 全部证据见 `docs/analysis/2026-08-10-fill-has-a-consumer-after-all.md`。
...
2. ~~**E 不做**，写 negative note 封存~~ **E 已推翻（08-10）**：不用等「带填充的
   fixture」，语料里就有——两张图 15 个被填充的封闭区域。它现在是**唯一还能给画面
   加东西的一项**，但便宜的那一头在消费侧（OCS 得先愿意发 `igBoundary2d` 面），
   不在解码侧。
...
**Phase 39 三项全部有结论。** D 已正确处理（拒收哨兵 + 回退可见）、F 是伪命中已撤回、
E 的封存被推翻——它成了唯一还能给画面加东西的一项，接续动作见下。
...
**原计划**：写 negative note 封存，说清「六图 Sheet 流无填充几何、36 条填充样式无
引用、占比 0」；翻案的最便宜验证是先证「某条几何的 `index` 指到 `0x002A`/`0x002B`」
再谈解码。

**实际执行**：直接跑了那条「最便宜验证」，它就是判据本身——而它**成立**。
产物是 `docs/analysis/2026-08-10-fill-has-a-consumer-after-all.md`，不是 negative note。

**接续动作（未执行，另开）**，次序不能反：

1. **[OCS] 把 `igBoundary2d` 发成闭合区域**。这是门：解码器解出填充颜色而消费方不发面，
   画面依旧不变。它也是真实的设计决定——边界的成员线已经在画，再发面要处理面线重叠。
   前置：验证边界段组连起来确实是封闭环（本轮没验）。
2. **[pid-parse] 解 `0x002A` 载荷**（46 字节）。手法同 `0x002E` / `0x002F`：
   CLSID → `style.dll` vtable → 版本分发序列化器 → 字段偏移。
