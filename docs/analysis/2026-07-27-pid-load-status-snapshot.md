...
>
> **2026-08-10 补测**（`examples/probe_text_height_fallback`）：把回退量化到条。
> 全语料 184 条文字记录 159 条解出、25 条回退；DWG-0201 只有 1 条回退，
> 它那 9 条 2.5mm 里至少 8 条是图纸自己声明的值。回退集中在工艺管道-1（43 条里 16 条）。
>
...
1. ~~**文字样式（字高/字体/旋转）**~~ **字高已基本关闭（Phase 37 + 2026-08-10 重测）**。
   原文（存档）：Phase 35-D 已证明字高**不在** `igTextBox` 记录内（146 条记录全偏移
   扫描无命中），12 字节 trailer 末 4 字节大端 u32 是跨图纸稳定的**样式 id**；
   样式表应在 `/StyleCluster`（`0x005A`），Phase 29 判定其 prefix 无 parser 证据
   → 卡在需要 native reader。
   **2026-08-10 实测**：两跳链路（`igTextBox.index` → `0x002D +38` → `0x002C +42`）
   全语料 **184/184 走通**，159 条拿到真字高，25 条被拒——被拒的全部指向同一条
   0.254mm（0.01″）模板样式，四张图里除身份字段外逐字节相同，且段落样式与文字记录
   里都没有第二个字高来源。拒收是正确行为，OCS 侧保留 2.5mm 回退并已在导入日志
   点名。见 `2026-08-10-text-height-residue-is-one-sentinel-not-version-2.md`。
   字体名（「最长 UTF-16 串」启发式）与旋转仍未定位。
...
5. ~~**离群实体**~~ **已关闭（2026-08-10）**。原文：实测四张图**可见图层零离群**，
   探针报的都在隐藏层上（`PID-UNRESOLVED` 的单位线、`PID-SYMBOL-LABEL` 的页外标签）。
   **`PID-UNRESOLVED` 那几条单位线现已从源头消失**：它们不是解错的记录，是
   `igSmartFrame2d` 页幅长宽比 `1/√2` 的高两字节被扫描式解码器当成了类型码
   `0x3FE6`。`decode_primitive_lines` 改按链成员资格取记录后，全语料零命中，
   OCS 的 `PID-UNRESOLVED` 图层与 `unresolved_unit_line` 特判一并删除。见
   `2026-08-10-gline2d-is-the-iso-page-ratio-not-a-record.md`。
