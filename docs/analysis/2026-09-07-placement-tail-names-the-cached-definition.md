...
- ~~缓存存储自己的 `StyleCluster`（如 `/JSite145/StyleCluster` 29 条）未接：缓存本体现在不带
  ...
  **2026-09-20 已开单并两侧接上**（OpenCADStudio 计划 `docs/plans/2026-09-20-a-cached-body-carries-its-own-stroke-styles.md`
  E1 pid-parse `7e69b8a`：`PidSymbolDefinition::primitive_styles` / `visible_strokes()`、`JSite::stroke_styles`、`PrimitiveStyle::dash_mm`；
  E2 OCS `3b8af2ed`：缓存本体逐笔底涂，虚线由本体自己的样式决定、颜色线宽仍按放置——57 笔虚线画对，`pid_import` 54 → 55）。
