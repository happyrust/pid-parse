...
- ~~缓存存储自己的 `StyleCluster`（如 `/JSite145/StyleCluster` 29 条）未接：缓存本体现在不带
  ...
  **2026-09-20 已开单并在 pid-parse 侧接上**（OpenCADStudio 计划 `docs/plans/2026-09-20-a-cached-body-carries-its-own-stroke-styles.md`
  E1：`PidSymbolDefinition::primitive_styles` / `visible_strokes()`、`JSite::stroke_styles`、`PrimitiveStyle::dash_mm`；OCS 侧 E2 待做）。
