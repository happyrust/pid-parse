...
- ~~放置实例的实际参数在文件的哪一处：五个实例四个被拉过，值不在缓存里。~~ **2026-09-20 找到了**：在实例存储自己的
  `0x00ED JFlavorHolder` 里，按 `JSymbolInformation` 变量顺序一列 `f64`（Manifold [0.057909, 0.1143, 0.03559]、Black Box
  [0.0127, 0.113927, 0.0127, 0.078067]，与实例几何逐个数对上）；见
  `2026-09-20-jflavorholder-carries-the-placed-instances-parameters.md`。§5 第 1 条「实例那份 `SymbolInformation` 不是实例的参数」
  仍对，§7「放置实例身上没有属于它的尺寸值」的前半句从此不成立——值在 flavour holder 里，未解码进 DTO。
