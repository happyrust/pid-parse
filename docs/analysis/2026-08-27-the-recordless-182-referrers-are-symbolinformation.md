...
### 2.1 反汇编侧独立确认，四个类名一次拿到

上面的身份是语料推出来的。把这四个 type code 送进 §4 那张
`radsrvitem.dll!dword_5667B068` → CLSID → `jutil.dll` RAD 注册表的链子
（`tools/psm_type_clsid.py`，每次重新联立 `(base, stride)`），**两条路对上了**：
...
`0x00BD = JSymbolInformation` 与 `PSMroots` 那个字符串 `SymbolInformation`
**完全独立地对上**（一个是文件里的名字表，一个是反汇编的类型表 → COM 注册表），
身份从此是 native-reader 级，不再只是语料推断。同轮复核了五个已发布锚点
（`0x00CE`/`0x005A`/`0x00FA`/`0x0013`/`0x003D`），全部一字不差地复现。

**这也把结构读顺了，同时修掉一个我自己的措辞。** `0x00C7` 不是什么「连接点叶子」，
它就是**表达式子系统里的一个双精度值对象**——24 字节里那个 `f64` 就是它的值；
`0x00EA` 是装这些值的**变量表**。所以这一族的真实形状是：

> `JSymbolInformation` 带一张**命名变量表**（`Left` / `Right` / `Bottom` / `Top`），
> 每个变量的值同时作为一条 `Double Value Object` 单独落盘，由一条
> `Variables Object` 收成一组；空间表那条 tag-182 入边的意思就是
> **「这个 `JSymbolInformation` 引用了这个变量」**。

下文 §4.1 里出现的「命名变量」一律指这个；`0x006F` 是每个变量的第二个引用者，
是 `jengine` 的关系对象，本轮没展开。
...
`0x00BD` 两种形状共用 44 字节头，**`+14` 的 `u16` 等于 `0x0010` 就说明后面还有变量
表**（35 条短桩和 2 条 50 字节变体那里都是 0）：

```text
+0   u32  oid
+4   u32  parent_ref
+8   u32  0
+12  u16  kind        —— 3（带变量表的形状）/ 11（50 字节变体）
+14  u16  0x0010 才有下文
+16  f64  尺寸 A      —— 短桩上为 0，几何含义未定
+24  f64  尺寸 B      —— 同上
+32  f64  0
+40  u32  4           —— 常量
--- 只在 +14 == 0x0010 时 ---
+44  u32  变量数
     变量数 × {
        u8   1 ; u32 1
        f64  值
        u16  字符数 ; UTF-16LE 变量名   —— 只有 Left / Right / Bottom / Top
        u32  该变量的 `Double Value Object`（`0x00C7`）oid
     }
```

**8 条长形按这个读法全部吃到最后一个字节，零剩余**（44 + 4 + Σ(19 + 2×字符数) 精确
等于 payload 长度）；24 个变量的名字只有四个侧向，前导两个字全是 `1`。而且**内联表
和独立值对象对得上**：内联的 `f64` 等于它所指 `0x00C7` 记录自己 `+12` 的 `f64`，
**12/12**（另外 12 个变量所在的存储压根没有 `0x00C7` 记录，见下）——按 §2.1 的类名
这本来就该相等，因为 `0x00C7` 就是那个双精度值本身。

最要紧的是这条**二分**——全语料 203 条 `0x00C7`：

| | 计数 |
|---|---|
| 被某条**活着的长形** `0x00BD` 列进变量表 | 12 |
...
的记录**就是本该列出这些变量的那条 `JSymbolInformation` 长形**——不是「一类不明
对象」，是一条形状已知、字段已解、类名已定的记录。棘轮：
...
- **这一族的两半是各自独立落盘的，两边都能缺。** `JSite793` 有 188 条 `0x00C7` /
  ...
  一到两条长形 `0x00BD`（变量表齐全）却**一条 `0x00C7`/`0x00EA` 都没有**——上面
  那 12 个「解析不到值对象」的变量正出自这三个存储。既然反方向也成立，就不必用
  「删了没清」来解释任何一边。
...
- tag 182 至少盖三个记录家族：`0x0067`（`_SupportOnlyList`，顶层）、`0x00BD`
  （`JSymbolInformation`，站点内）、`0x006F`（`jengine` 的关系对象）。所以 §3.3
  那张表里的「182 = `_SupportOnlyList`」只是这个类的一个代表名，不是全部。
- §4 的 type code 表新增四行（§2.1），其中 `0x00C7` = `Double Value Object`、
  `0x00EA` = `Variables Object` 说明**表达式子系统的对象会和图形对象混在同一条
  记录链里**，按 type code 分家族时别默认「一条记录就是一个图元」。
...
**probe（同日补）**：`examples/probe_symbol_information_family_shape.rs`。它按**声明
长度走链**（解码器看不到的那一层——`scan` 是逐字节滑的），再拿两边对帐，于是多验出
一条本来没验的事：**每一条被解码器接受的记录都正好落在一个链记录的起止上，零例外**。
逐字节滑窗既没蒙出假记录，变长尾也没差一个字节。它同时打印每个站点的实物清单
（符号的变量表、关系的签名与公式）。
...
- 公式串开头 `0E`、`+12` 那个恒定 GUID、操作数槽里的 `0145EEC0-…`，三处仍未认；
  两个 GUID 都不在 `jutil.dll` 注册表里，多半是接口 IID。
