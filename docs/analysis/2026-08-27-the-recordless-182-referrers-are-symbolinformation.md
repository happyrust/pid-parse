...
- **(a) 长形记录被删了、反指没清**——和 `DWG-0201` 那条悬空 tag-249 边同一机制，
  只是规模大。反对它的唯一一条是「这些 index 不在自由表」，而 §3 已经说明在这个
  段里这条反对很弱（8192 个 index 只有 79 个在自由表）。
- **(b) 那一层在这个文件里本来就没落盘。**

分不开这两条，就还不能说「记录去哪了」。要分开，得找一个既有长形又有短桩的存储去
比，或者从写侧看长形的落盘条件。
...
- 把 `0x00BD` / `0x006F` / `0x00C7` / `0x00EA` 这一族解出来命名，**并且分开
  `0x00BD` 的 44 字节短桩和长形**（§4）——长形才是引用形，两者的字段差在哪、长形的
  落盘条件是什么，是分辨 §4 (a)/(b) 两种读法的关键。`0x00BD` 长形带 UTF-16 名字
  （语料里见到 `Left`），`0x00C7` 是 24 字节叶子（一个 f64 + `1` + `15`），形状上
  像「符号 + 连接点」，但没有独立证据，本文不给名字。
