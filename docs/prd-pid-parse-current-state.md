...
2. **Sheet 深层几何未完成**  
   ...
   Phase 23 后的坐标合同边界是：
   - `NormalizedPidGeometry.page_dimensions_mm` 只是 template-derived page-size evidence。
   - `PidCoordinateContext.page_transform` 仍应保持 `Unavailable`，直到找到完整 source record / scalar source / decoded semantics。
   - 下游（包括 H7CAD）不应把 source coordinates 直接当作 page-space 或 viewport pixels。
   - coordinate/page metadata 的 top evidence 仍是 investigation-only，不代表 decoded transform。
