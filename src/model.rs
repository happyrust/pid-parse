...
/// Reverse provenance link from one [`PidObject`] to the
/// `AttributeRecord` inside `Unclustered Dynamic Attributes` that first
/// carried its `DrawingID`. Lets downstream callers jump from the object
/// graph back to the decoded DA record without re-scanning the stream.
///
/// Lookup is keyed on `PidObject.drawing_id` against
/// `AttributeField { name = "DrawingID" | "DrawingNo", value = Text(..) }`.
/// The first attribute record that announces a given `DrawingID` wins
/// (subsequent duplicates are not surfaced here).
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct ObjectSourceRef {
    /// 32-char hex drawing id of the originating `PidObject`.
    pub drawing_id: String,
    /// `class_name` of the DA attribute record (e.g. `"Instrument"`,
    /// `"Drawing"`, `"Nozzle"`), copied from
    /// [`AttributeRecord::class_name`].
    pub class_name: String,
    /// Index into `DynamicAttributesBlob.attribute_records` that matched
    /// this drawing id first.
    pub attribute_record_index: usize,
    /// Confidence label of that attribute record — mirrors
    /// [`AttributeRecord::confidence`] (e.g. `"heuristic"` / `"decoded"`).
    pub confidence: String,
    /// `true` when the `PidObject` itself carries a DA `record_id` (i.e.
    /// the DA trailer was fully matched during decoding). Serves as a
    /// quick "is this link trustworthy?" signal without reaching back
    /// into the trailer vector.
    #[serde(default)]
