...
**2026-09-07 续（W4 关闭）**：`igCircle2d` / `igArc2d` 有了解码器（四主图 12 + 12，
与 aux_hi 名册一致）；逐值比对证明嵌套 `LdcSite` 是**内嵌符号定义缓存**（20/24 曲线
与 `.sym` 库逐字节相同）；随后在 `igSymbol2d` payload 末尾找到 `(JSheet, LdcSite)`
两个字，经缓存 spacemap tag-183 → 管理器 → 图层拿到本体，**107/107 放置全部解析**。
本体经 `NormalizedPidGeometry::symbol_definitions` 交渲染端按放置矩阵上页面，
